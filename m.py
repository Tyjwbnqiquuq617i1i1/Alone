...
def get_random_image_url():
    ...
    return random.choice(random_image_urls)
...
def start_attack_reply(message, target, port, time):
    ...
    𝐓𝐀𝐑𝐆𝐄𝐓: {target}
    𝐏𝐎𝐑𝐓: {port}
    𝐃𝐔𝐑𝐀𝐓𝐎𝐈𝐍: {time} Seconds
    """

    # Get a random image URL
    random_image_url = get_random_image_url()

    # Send the attack message along with the image
    bot.send_photo(message.chat.id, random_image_url, caption=response)
